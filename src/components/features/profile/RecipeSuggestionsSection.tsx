...
const BlacklistedIngredientsSection = () => {
  const ingredients: Ingredient[] = [
    ...
    {name: "Shellfish", id: "5", createdAt: new Date(), updatedAt: new Date()},
    {name: "Eggs", id: "6", createdAt: new Date(), updatedAt: new Date()},
    {name: "Fish", id: "7", createdAt: new Date(), updatedAt: new Date()},
  ]

  const [blacklistedIngredients, setBlacklistedIngredients] = useState<Ingredient[]>([]);

  const handleIngredientClick = (ingredient: Ingredient) => {
    console.log(ingredient);
    if (blacklistedIngredients.some((item) => item.id === ingredient.id)) {
      setBlacklistedIngredients(blacklistedIngredients.filter((item) => item.id !== ingredient.id));
    } else {
        ...
        <CommandList>
          <CommandGroup>
            ...
            {ingredients.map((ingredient) => (
              <CommandItem key={ingredient.id} className="gap-2" onSelect={() => handleIngredientClick(ingredient)}>
                {ingredient.name}
                <CheckIcon
                  className={cn(
                    "ml-auto h-4 w-4",
                    blacklistedIngredients.some((item) => item.id === ingredient.id) ? "opacity-100" : "opacity-0"
                  )}
                />
              </CommandItem>
            ))}
...
const BlacklistedIngredient = ({ingredient}: {ingredient: Ingredient}) => {
  return (
    <div className="py-1 px-2 rounded-md bg-slate-100">
      {ingredient.name}
    </div>
  )
};